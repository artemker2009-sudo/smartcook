...
/**
 * Чтение переменной с обязательным trim. Через него идут ВСЕ пять
 * ROBOKASSA_*, и это не косметика: пароли попадают в env копипастой из кабинета
 * Robokassa, а пробел или перенос строки на конце меняет md5 целиком. Ошибка
 * при этом выглядит не как «опечатка в пароле», а как «Robokassa не работает»:
 * код 29 на странице оплаты, тот же, что при полностью чужом пароле. Лишний
 * символ глазами в интерфейсе Vercel не виден.
 */
