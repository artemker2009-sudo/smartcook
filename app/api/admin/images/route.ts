...
// Одна генерация на запрос (клиент в админке гонит батч последовательно, по
// одному) → показываем прогресс.
//
// Лимит поднят с 60 вместе с переездом на gpt-image-2/medium: замер показал
// 63 секунды на картинку, то есть прежнего лимита не хватало бы ровно на
// середине генерации — деньги списаны, картинки нет.
