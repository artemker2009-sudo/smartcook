...
export async function POST(req: Request) {
  try {
    // Получаем флаг isVariant (значит пользователь нажал "Другой вариант")
    const { query, sessionId, isVariant } = await req.json();

    let taskDescription = `Твоя задача — выдать ИДЕАЛЬНО ТОЧНУЮ технологическую карту блюда ровно на 1 ПОРЦИЮ.`;
    
    // Если просят вариант, меняем задачу
    if (isVariant) {
      taskDescription += ` 
      ВАЖНО: Пользователь уже видел классический рецепт "${query}" и хочет ДРУГОЙ ВАРИАНТ.
      Предложи альтернативную версию:
      - Измени способ приготовления (например, жареное вместо вареного).
      - Или добавь интересный ингредиент (сыр, специи, соус).
      - Или сделай "ленивый" вариант.
      - В названии укажи особенность (например, не просто "Омлет", а "Омлет с сырной корочкой").
      `;
    }
    ...
    const systemPrompt = `
      ...
      {
        "title": "Название (с уточнением, если это вариант)",
