...
// ВАЖНО: здесь стоит проверка на нативную оболочку, а НЕ общий флаг
// useCanPromptInstall, хотя соседние точки входа переведены на него.
//
// Этот компонент не только рендерит карточку — он ведёт трекинг установок, и
// главная его цель, pwa_standalone_launch, срабатывает ровно там, где общий
// флаг сказал бы «молчи»: при первом запуске с домашнего экрана. Спрятать узел
// в standalone значит потерять единственный прокси iOS-установок. Показ самой
// карточки закрыт внутри InstallPromptCard — там общий флаг и стоит.
//
// Обёртка вынесена отдельным компонентом намеренно: ранний return внутри
// PWAInstallInner менял бы число вызванных хуков между первым рендером и
// следующим, а это ошибка React.
