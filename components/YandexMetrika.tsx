...
import { currentInstallEnv } from "@/lib/installEnv";
...
// Режим отображения приложения — параметр каждого хита Метрики.
//
// Тонкая обёртка над единым определением среды (lib/installEnv): сама формула
// живёт там и одна на всё приложение. Здесь остаётся только маппинг в те два
// значения, которые Метрика собирает исторически, чтобы старые отчёты не
// разъехались: standalone — установленное PWA или TWA из RuStore, browser —
// всё остальное. Нативная оболочка (Capacitor) намеренно попадает в browser,
// как и до появления installEnv: смешивать нативный трафик с PWA в одном
// значении нельзя, его считают отдельно.
export function getDisplayMode(): "standalone" | "browser" {
  const env = currentInstallEnv();
  return env === "pwa" || env === "twa" ? "standalone" : "browser";
}
