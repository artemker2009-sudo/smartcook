...
//
// 2. RECEIPT ПОДПИСЫВАЕТСЯ УЖЕ URL-КОДИРОВАННЫМ. Раздел «Фискализация»:
//    «перед добавлением в строку для подписи значение Receipt нужно
//    URL-кодировать». В md5 уходит строка вида %7B%22items%22%3A… — ровно та,
//    что стоит в примере документации; сверено побайтово тестом
//    («encodeReceipt даёт ту же строку, что в примере документации»).
//    Ту же строку кладём и в адрес. Отсюда же ручная сборка query:
//    URLSearchParams закодировал бы её ВТОРОЙ раз.
//
//    ЧТО ЗДЕСЬ ЕЩЁ НЕ ПРОВЕРЕНО ЖИВОЙ ОПЛАТОЙ. Пример в документации — форма
//    POST, где транспортный слой кодирования добавляет браузер, а сервер
//    Robokassa его снимает. У нас ссылка GET, и такого слоя нет: вопрос, ждёт
//    ли Robokassa в GET-адресе Receipt, закодированный ВТОРОЙ раз, остаётся
//    открытым. 26.09.2026 проверены оба варианта на живом магазине — оба дали
//    код 29, но тогда не сходился сам Пароль #1 (тестовый режим требует
//    ОТДЕЛЬНЫЙ тестовый пароль), поэтому опыт ничего не сказал про кодирование.
//    Когда пароль будет верным и 29 останется — начинать надо отсюда:
//    закодировать Receipt в адресе вторым проходом (подпись не меняется) или
//    уводить человека формой POST, как в документации.
//
