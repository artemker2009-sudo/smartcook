...
export function useAuthModal(options: Options = {}) {
   ...
   *
   * ВСЕ переносы гостевого опыта в аккаунт живут ЗДЕСЬ, а не в экране, который
   * позвал хук. Раньше так было только со списками покупок, а перенос профиля
   * вкуса был подключён лишь в SearchApp — и у тех, кто регистрировался через
   * /profile, аллергии не просто не переезжали, а стирались при первой же
   * правке (подробности в lib/tasteProfile.ts). Теперь точка одна, и следующий
   * экран с авторизацией не заведёт четвёртую дыру.
   *
   * Ничего из этого не ждём: следом за регистрацией показывается код
   * восстановления — ровно один раз за всю жизнь аккаунта, — и задерживать тот
   * экран сетевым запросом нельзя. Все три переноса best-effort: что не доехало
   * сейчас, доедет при следующем запуске (у рецептов для этого есть подметалка
   * на экранах истории).
   *
   * Исключение — профиль вкуса: его ждём. Он пишет в user_metadata, и экраны
   * подхватывают результат через onAuthStateChange; к тому же это аллергии,
   * то есть единственный из трёх переносов, у которого цена ошибки — здоровье.
   */
